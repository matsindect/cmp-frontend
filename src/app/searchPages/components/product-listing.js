...
export default function MediaControlCard({ business, productCategory, sector }) {
  ...
  return (
    <>
      {products && products.filter(p => p.business_types.some(bt => bt.name === business.name) 
      && p.sectors.some(s => s.name === sector.name)
      && p.product_categories.some(pc => pc.name === productCategory.name)).slice(0, 22).map((product) => {
          return (
                  ...
                  <div className={classes.details}>
                    <CardContent className={classes.content}>
                      ...
                      <Typography variant="subtitle1" color="textSecondary">
                        {product.product_attributes.map((attrr) => {
                          return (
